...
#define WIFI_DRIVER_MODULE_PATH    "/system/lib/modules/mt5931.ko"
#endif
...
static const char SUPPLICANT_NAME[]     = "mtk_wsupplicant";
static const char SUPP_PROP_NAME[]      = "init.svc.mtk_wsupplicant";
static const char P2P_SUPPLICANT_NAME[] = "mtk_psupplicant";
static const char P2P_PROP_NAME[]       = "init.svc.mtk_psupplicant";
static const char AP_DAEMON_NAME[]      = "mtk_ap_daemon";
static const char AP_PROP_NAME[]        = "init.svc.mtk_ap_daemon";
static const char SUPP_CONFIG_TEMPLATE[]= "/system/etc/wifi/wpa_supplicant_mt5931.conf";
static const char P2P_CONFIG_TEMPLATE[] = "/system/etc/wifi/p2p_supplicant_mt5931.conf";
...
int is_wifi_driver_loaded_mt5931() {
...
int wifi_load_driver_mt5931()
{
#ifdef WIFI_DRIVER_MODULE_PATH
    char driver_status[PROPERTY_VALUE_MAX];
    int count = 100; /* wait at most 20 seconds for completion */

    if (is_wifi_driver_loaded_mt5931()) {
        //Enable power HERE
        //wifi_set_power(1);
        return 0;
    }
...
int wifi_unload_driver_mt5931()
{
    //usleep(200000); /* allow to finish interface down */
    
    ALOGD("wifi_unload_driver_mt5931");   
...
int ensure_entropy_file_exists_mt5931()
{
    int ret;
    int destfd;

    ALOGV("ensure_entropy_file_exists_mt5931");
...
static int update_ctrl_interface(const char *config_file) {
...
static int ensure_config_file_exists_common(const char *config_file, const char *config_template)
{
    char buf[2048];
    int srcfd, destfd;
    struct stat sb;
...
int ensure_config_file_exists_mt5931(const char *config_file)
{
 ...
 */
static void wifi_wpa_ctrl_cleanup(void)
{
    DIR *dir;
    struct dirent entry;
...
int wifi_start_supplicant_mt5931(int p2p_supported)
{
    ...
    ALOGD("wifi_start_supplicant_mt5931 [%d]", p2p_supported);
    ...
    if (p2p_supported) {
        ...
        if (ensure_config_file_exists_mt5931(P2P_CONFIG_FILE) < 0) {
    ...
    if (ensure_config_file_exists_mt5931(SUPP_CONFIG_FILE) < 0) {
        ALOGE("Wi-Fi will not be enabled");
        return -1;
    }

    if (ensure_entropy_file_exists_mt5931() < 0) {
...
int wifi_ap_start_supplicant()
{
    ...
    //Start ap daemon
    if (wifi_start_supplicant_mt5931(2) < 0) {
        ret = -1;
    }
    
    return ret;
}
int wifi_stop_supplicant_mt5931(int p2p_supported)
{
    char supp_status[PROPERTY_VALUE_MAX] = {'\0'};
...
int wifi_ap_stop_supplicant()
{
...
#if 0
    if(wifi_stop_supplicant_mt5931(0) < 0) {
        ret = -1;
    }    
#else    
    //Stop p2p_supplicant
    wifi_connect_to_supplicant_mt5931(WIFI_AP_INTERFACE);
    if(wifi_command_mt5931(WIFI_AP_INTERFACE, "TERMINATE", buffer, &len) < 0) {
        if(wifi_stop_supplicant_mt5931(0) < 0) {
            ret = -1;
        }
    }
    ...
    wifi_close_supplicant_connection_mt5931(WIFI_AP_INTERFACE);
#endif
    
    return ret;
}

static int wifi_connect_on_socket_path(int index, const char *path)
{
    char supp_status[PROPERTY_VALUE_MAX] = {'\0'};
...
int wifi_connect_to_supplicant_mt5931(const char *ifname)
{
    char path[256];
    ...
    ALOGD("[%s]wifi_connect_to_supplicant_mt5931", ifname);
    ...
    while (count-- > 0) { 
        ...
        if(ret == 0) {
            ALOGV("[%s]wifi_connect_to_supplicant_mt5931 OK", ifname);
            break;
        }
        ALOGV("[%s]wifi_connect_to_supplicant_mt5931 Retry, sleep %dus", ifname, SUPP_CONNECT_DELAY);
        usleep(SUPP_CONNECT_DELAY);
    }
    
    if(count <= 0) {
        ALOGE("[%s]wifi_connect_to_supplicant_mt5931 Failed!!", ifname);
    }
   
    return ret;
}

static int wifi_send_command(int index, const char *cmd, char *reply, size_t *reply_len)
{
    int ret;
...
static int wifi_ctrl_recv(int index, char *reply, size_t *reply_len)
{
    int res;
...
static int wifi_wait_on_socket(int index, char *buf, size_t buflen)
{
...
int wifi_wait_for_event_mt5931(const char *ifname, char *buf, size_t buflen)
{
    int ret = 0;
    
    //ALOGD("[%s]wifi_wait_for_event_mt5931", ifname);
    if (is_primary_interface(ifname)) {
...
void wifi_close_supplicant_connection_mt5931(const char *ifname)
{
    char supp_status[PROPERTY_VALUE_MAX] = {'\0'};
    int count = 50; /* wait at most 5 seconds to ensure init has stopped stupplicant */

    ALOGD("[%s]wifi_close_supplicant_connection_mt5931", ifname);
...
int wifi_command_mt5931(const char *ifname, const char *command, char *reply, size_t *reply_len)
{
    int ret = 0;
    ...
    if (ret == 0) {
        ...
        //ALOGE("[%s]SET \"%s\" Timeout",ifname, command);
    }
    else {
        //ALOGE("[%s]SET \"%s\" Failed",ifname, command);
    }
    
    return ret;
}

const char *wifi_get_fw_path_mt5931(int fw_type)
{
    ALOGV("wifi_get_fw_path_mt5931 [%d]", fw_type);
    switch (fw_type) {
...
int wifi_change_fw_path_mt5931(const char *fwpath)
{
    ALOGD("wifi_change_fw_path_mt5931 [%s]", fwpath);
